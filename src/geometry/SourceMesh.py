...
class SourceMesh:
    ...
    def __init_from_mesh_data(self):
        ...
        # assign the pre-computed Poisson solver to the source mesh
    ...
    def load(self, source_v=None, source_f=None):
        ...
        if source_v is not None and source_f is not None:
            self.mesh_processor = MeshProcessor.MeshProcessor.meshprocessor_from_array(
                source_v,source_f, self.source_dir, self.__ttype, cpuonly=self.cpuonly, load_wks_samples=self.__use_wks, load_wks_centroids=self.__use_wks
            )
        else:
            if os.path.isdir(self.source_dir):
                ...
                self.mesh_processor = MeshProcessor.MeshProcessor.meshprocessor_from_file(
                    self.source_dir,
                    self.__ttype,
                    cpuonly=self.cpuonly,
                    load_wks_samples=self.__use_wks,
