...
class PoissonSystem:
    ...
    def __init__(
        ...
        cpu_only: bool = False,
        no_factorization: bool = False,
        rel_pair_i: Optional[Int[Tensor, "num_pair"]] = None,
        rel_pair_j: Optional[Int[Tensor, "num_pair"]] = None,
        rel_offsets: Optional[Shaped[Tensor, "num_pair 3"]] = None,
        rel_lambda: float = 1.0,
        rel_pair_w: Optional[Shaped[Tensor, "num_pair"]] = None,
    ) -> None:
        """
        ...
        self.rel_pair_i = rel_pair_i
        self.rel_pair_j = rel_pair_j
        self.rel_offsets = rel_offsets
        self.rel_lambda = float(rel_lambda)
        self.rel_pair_w = rel_pair_w
        self.has_rel = (
            rel_pair_i is not None
            and rel_pair_j is not None
            and rel_offsets is not None
            and int(rel_pair_i.shape[0]) > 0
            and self.rel_lambda > 0.0
        )
    ...
    def _build_poisson_system(self) -> None:
        ...
        self.is_constrained = self.anchor_inds is not None or self.has_rel
        ...
        if self.anchor_inds is not None:
            ...
            self.indicator_product = None
        if self.has_rel:
            self._build_relative_constraints()
        else:
            self.rel_product = None
            self.rel_rhs = None
        ...
        if not self.no_factorization:
            try:
                ...
                print("[PoissonSystem] Cholesky factorization succeeded")
            except Exception as exc:
                self.cholesky_ok = False
                print(f"[PoissonSystem] Cholesky factorization FAILED: {exc}")
                raise
        else:
    ...
    def _solve_constrained(
        self,
        constraint: Optional[Shaped[Tensor, "num_handle 3"]] = None,
        trans_mats: Optional[Shaped[Tensor, "num_face 3 3"]] = None,
    ) -> Shaped[Tensor, "num_vertex 3"]:
        # TODO: Clean up the code
        # compute the RHS of the system
        ...
        if (
            self.indicator_matrix is not None
        ...
        if self.rel_rhs is not None and self.rel_lambda > 0.0:
            rhs = rhs + self.rel_lambda * self.rel_rhs
    ...
    def _build_relative_constraints(self) -> None:
        """Soft (Vi - Vj) ≈ offset constraints. Couples mesh components in L^T L."""
        nv = int(self.v_src.shape[0])
        ia = self.rel_pair_i.detach().cpu().numpy().astype(np.int64).reshape(-1)
        ib = self.rel_pair_j.detach().cpu().numpy().astype(np.int64).reshape(-1)
        n = int(ia.shape[0])
        if self.rel_pair_w is None:
            w = np.ones(n, dtype=np.float64)
        else:
            w = self.rel_pair_w.detach().cpu().numpy().astype(np.float64).reshape(-1)
            if w.shape[0] != n:
                w = np.ones(n, dtype=np.float64)
        rows = np.concatenate([ia, ib, ia, ib])
        cols = np.concatenate([ia, ib, ib, ia])
        data = np.concatenate([w, w, -w, -w]).astype(np.float64)
        k = coo_matrix((data, (rows, cols)), shape=(nv, nv)).tocsr()
        k.sum_duplicates()
        k_coo = k.tocoo()
        self.rel_product = SparseMat.from_coo(k_coo, self.torch_dtype).to(self.device)
        off = self.rel_offsets.detach().to(dtype=self.torch_dtype, device=self.device)
        w_t = torch.as_tensor(w, dtype=self.torch_dtype, device=self.device).unsqueeze(-1)
        rhs = torch.zeros(
            (nv, 3), dtype=self.torch_dtype, device=self.device
        )
        ia_t = self.rel_pair_i.to(device=self.device, dtype=torch.long)
        ib_t = self.rel_pair_j.to(device=self.device, dtype=torch.long)
        rhs.index_add_(0, ia_t, off * w_t)
        rhs.index_add_(0, ib_t, -off * w_t)
        self.rel_rhs = rhs
    ...
    def _compute_matrix_to_factorize(self):  # NOTE: Temporarily remove type annotation
        ...
        if self.is_constrained:
            ...
            if self.indicator_product is not None:
                indicator_product: coo_matrix = self.indicator_product.to("cpu").to_coo()
                mat_to_fac = (
                    mat_to_fac + self.constraint_lambda * indicator_product
                )
            if self.rel_product is not None:
                rel_product: coo_matrix = self.rel_product.to("cpu").to_coo()
                mat_to_fac = mat_to_fac + self.rel_lambda * rel_product
