...
class PoissonSolver:
    ...
    def jacobians_from_vertices(
        self,
        V: Shaped[Tensor, "batch_size num_vertex 3"],
    ) -> Shaped[Tensor, "batch_size num_face 3 3"]:
    ...
    def solve_poisson(self,jacobians): 
        ...
        sol = _predicted_jacobians_to_vertices_via_poisson_solve(
            self.my_splu,
            self.sparse_rhs,
            jacobians.transpose(2, 3).reshape(jacobians.shape[0], -1, 3, 1).squeeze(3).contiguous(),
        )
        # torch.cuda.synchronize()
...
def poisson_system_matrices_from_mesh(
    ...
    '''
    Computes matrices involving in Poisson's equation from the given mesh.
    
    Args:
        V: Vertex coordinates
        F: Vertex indices constituting each triangle
        dim: for now always 3 :)
        ttype: Type of tensor (e.g., float, double)
    ...
    grad = igl.grad(vertices, faces)  # (3F, V)
    ...
    # NOTE: There is no libigl function that directly computes Laplacian for us
