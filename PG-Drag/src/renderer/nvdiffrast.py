...
@jaxtyped(typechecker=typechecked)
def render_vertex_attribute(
    v: Float[torch.Tensor, "V 3"],
    f: Int[torch.Tensor, "F 3"],
    vertex_attr: Float[torch.Tensor, "V C"],
    cam2world: Float[torch.Tensor, "4 4"],
    proj_mat: Float[torch.Tensor, "4 4"],
    img_height: int,
    img_width: int,
    ...
    device: torch.device = torch.device("cuda"),
    ss_scale: float = 1.0,
) -> Float[torch.Tensor, "H W C"]:
    """
    Renders vertex attributes (e.g., SDS mask) via differentiable rasterization.
    
    Args:
        v: Vertices (V, 3)
        f: Faces (F, 3)
        vertex_attr: Vertex attributes (V, C) - e.g., SDS weights per vertex
        cam2world: Camera extrinsic matrix
        proj_mat: Projection matrix
        img_height: Image height
        img_width: Image width
        bg_value: Background value for pixels not covered by mesh
        use_opengl: Flag for using OpenGL backend
        device: Device where the computation takes place
        ss_scale: Supersampling scale
    
    Returns:
        attr_img: Rasterized attribute image (H, W, C)
    """
    # Ensure all tensors are on the correct device
    v = v.to(device)
    f = f.to(device)
    vertex_attr = vertex_attr.to(device)
    ...
    vertex_attr = vertex_attr[None, ...]  # (1, V, C)
    attr_img, _ = dr.interpolate(vertex_attr, rast_out, f)
    
    # Anti-aliasing
    attr_img = dr.antialias(attr_img, rast_out, v_clip, f)
    
    # Set background value for pixels not covered by mesh
    fg_mask = (rast_out[..., 3:4] > 0.0).float()
    attr_img = attr_img * fg_mask + bg_value * (1.0 - fg_mask)
    
    # Flip images vertically (nvdiffrast convention)
    attr_img = torch.flip(attr_img, dims=[1])
    
    # Truncate batch dimension
    attr_img = attr_img[0, ...]  # (H_ss, W_ss, C)
    
    # Downsample if needed
    if ss_scale > 1.0:
        attr_img = attr_img.permute(2, 0, 1)  # (C, H_ss, W_ss)
        attr_img = F.interpolate(
            attr_img[None, ...],
            (img_height, img_width),
            mode="bilinear",
            align_corners=False,
        ...
        attr_img = attr_img.permute(1, 2, 0)  # (H, W, C)
